...
import { impliedAprPct, discountDeadline, recommendWithScenarios, isDiscountDeadlinePassed } from '../utils/calculator';
...
export class InvoicesService {
  ...
  async importCsv(fileBuf: Buffer, userId: string) {
    ...
    logger.info(`[InvoicesService] importCsv: userId=${userId} rows=${totalRows}`);
    ...
    logger.info(`[InvoicesService] import complete: userId=${userId} imported=${imported} skipped=${skipped} rows=${totalRows}`);
